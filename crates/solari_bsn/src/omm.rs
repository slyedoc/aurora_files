...
//! directly to a `VkMicromapEXT` build + a per-triangle OMM index buffer for the
//! NV-cluster CLAS attach — see the `solari-omm-cluster-path` plan.
...
pub fn bake(
    ...
    assert_eq!(alpha.len(), (width * height) as usize, "alpha size mismatch");
    ...
    let bake = unsafe {
        ...
        let index_histogram =
            slice_to_vec(result.index_histogram, result.index_histogram_count as usize);
