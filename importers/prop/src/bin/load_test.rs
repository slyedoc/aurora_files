//! Headless validation that a baked `.bsn` parses, resolves its meshes, and carries real
//! material factors — no GPU. Pass the asset root ABSOLUTELY; a relative one resolves against the
//! crate dir, not the workspace.
//!
...
use bevy::animation::animclip::{AnimatedScene, AnimationClipLoader};
...
fn main() {
    ...
    let scene_path = args
        .get(1)
        .cloned()
        .expect("usage: load_test <abs asset root> <scene.bsn>");
    ...
    app.register_type::<RaytracingMesh3d>()
        ...
        // The rigged/animated path: without this the loader hard-errors on the unknown type and
        // the whole scene fails, reporting 0 for every count below — which reads exactly like a
        // broken bake. Registering the real component keeps that honest.
        .register_type::<AnimatedScene>()
        // `register_asset_reflect` is what lets the `.bsn`'s bare path literal coerce into a
        // `Handle<AnimationClip>`; with only `init_asset` the loader reports "type mismatch".
    ...
    // Re-read: a large scene (zeroday spawns 8641 nodes) can still be empty when the wait loop
    // gives up, and the stale 0 then fails the run even though everything resolved.
    named = app
        .world_mut()
        .query_filtered::<(), With<Name>>()
        .iter(app.world())
        .count();
    ...
    println!("ANY texture handle set:             {any_texture}");
    let clip = app
        .world_mut()
        .query::<&AnimatedScene>()
        .iter(app.world())
        .next()
        .map(|c| c.0.path().map(|p| p.to_string()).unwrap_or_default());
    println!(
        "animclip marker:                    {}",
        clip.as_deref().unwrap_or("(none — static asset)")
    );

    // Textures are legal here (unlike the texture-free hoverboard) — a prop may ship maps. What
    // must hold is that the scene spawned, every mesh resolved, and materials aren't all defaults.
    let ok = named > 0 && meshes > 0 && cluster_meshes_loaded == meshes
        && (non_default_base > 0 || any_texture);
